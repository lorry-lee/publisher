...
def browser(request):
    ...
    resource_type, resource_type_path = support.get_resource_type_folder(request)
    ...
    xml_response = ElementTree.ElementTree(ElementTree.Element("Connector", {'command':command_name, 'resourceType':resource_type}))
    ...
    if None in (command_name, resource_type, folder_path):
        ...
        abs_path =  actual_path(settings.FCKEDITOR_CONNECTOR_ROOT, resource_type_path, folder_path)
        abs_url  =  actual_url(settings.FCKEDITOR_CONNECTOR_URL, resource_type_path, folder_path)

        xml_response.getroot().append(ElementTree.Element("CurrentFolder",{'path':folder_path,'url': abs_url,}))
        ...
        if (command_name == 'GetFolders'):
            ...
            return HttpResponse("""<script type="text/javascript">window.parent.frames['frmUpload'].OnUploadCompleted(%s)</script>""" % status)

        else:
            # unknown command
            xml_response.getroot().append(ElementTree.Element("Error", {'number':1, 'text':'blarf'}))

    xml_response.getroot().append(ElementTree.Element("Error", {'number':str(err_no), 'text':err_txt}))
...
def uploader(request):
    ...
    try:
        ...
        folder_path =  actual_path(settings.FCKEDITOR_CONNECTOR_ROOT, resource_type_path, '')

        # handle the upload
        status, filename = actions.file_upload(request, folder_path)

        # calculate the resulting file URL
        file_url  =  actual_url(settings.FCKEDITOR_CONNECTOR_URL, resource_type_path, '', filename)
    ...
    return HttpResponse("""<script type="text/javascript">window.parent.OnUploadCompleted(%s, '%s', '%s', '')</script>""" % (status, file_url, filename,))
